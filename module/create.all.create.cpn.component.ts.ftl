...
@Component({
  selector: 'app-${pojo.formatedUrl}-create-${role.name}',
  templateUrl: './${pojo.formatedUrl}-create-${role.name}.component.html'
})
export class ${pojo.name}Create${role.name?cap_first}Component extends AbstractCreateController<${pojo.name}Dto, ${pojo.name}Criteria, ${pojo.name}Service>  implements OnInit {
...
<#list pojo.fields as field>
    <#list field.typeAsPojo.fields as fieldRequiered>
        <#if fieldRequiered.requierd && fieldRequiered.typeAsPojo.ignoreFront == false>
...
<#list pojo.fieldsGeneric as fieldGeneric>
    ...
    <#if pojo.uploadOne>
    public save(): void {
        <#list 0..pojo.fields?size-1 as i>
             <#if pojo.fields[i].uploadOne>
        this.item.${pojo.fields[i].name} = JSON.stringify(this.fileTempDtosForOne[${i}]);
             </#if>
        </#list>
        super.save();
    }
...
<#if pojo.showPopupCreate==false>
    public navigateTo${pojo.name}List(){
        this.router.navigate(['/app/${pojo.formatedUrl?uncap_first}/list']);
    }
</#if>
...
<#list pojo.fields as field>
    <#if field.list>
        <#list field.typeAsPojo.fields as fieldRequiered>
            <#if fieldRequiered.requierd>
    public validate${field.name?cap_first}${fieldRequiered.name?cap_first}(){
            <#if fieldRequiered.simple>
        if (this.${field.name}Element.${fieldRequiered.name} == null) {
            <#elseif fieldRequiered.generic>
        if (this.${field.name}Element.${fieldRequiered.name} == null) {
            <#elseif fieldRequiered.list>
        if (this.${field.name}Element.${fieldRequiered.name} == null || this.${field.name?cap_first}.${fieldRequiered.name}.length === 0) {
            </#if>
...
<#list pojo.fieldsGenericIncludingInnerTypeInListField as fieldGeneric>
    ...
    const isPermistted = await this.roleService.isPermitted('${fieldGeneric.typeAsPojo.name}', 'add');
    if(isPermistted) {
...
<#list pojo.fieldsGenericIncludingInnerTypeInListField as fieldGeneric>
    ...
    get ${fieldGeneric.name}s(): Array<${fieldGeneric.typeAsPojo.name}Dto> {
        return this.${fieldGeneric.typeAsPojo.name?uncap_first}Service.items;
    }
    set ${fieldGeneric.name}s(value: Array<${fieldGeneric.typeAsPojo.name}Dto>) {
